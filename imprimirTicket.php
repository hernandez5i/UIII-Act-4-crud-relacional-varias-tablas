...
$sentenciaProductos = $base_de_datos->prepare("SELECT p.id_zapato, p.id_proveedor,p.precio, pv.cantidad
FROM zapatos p
INNER JOIN 
zapatos_vendidos pv
ON p.id = pv.id_zapato
WHERE pv.id_venta = ?");
$sentenciaProductos->execute([$id]);
$zapatos = $sentenciaProductos->fetchAll();
if (!$zapatos) {
    exit("No hay zapatos");
}
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td class="producto"><?php echo $producto->id_proveedor;  ?> <strong>$<?php echo number_format($producto->precio, 2) ?></strong></td>
